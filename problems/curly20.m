...
function  varargout = curly20( action, varargin )
...
case 'setup' % varargout = [ x0, fstar, xtype, xlower, xupper, clower, cupper, class, error ]
   ...
   if ( length( varargin ) )
      n = varargin{1};
      if ( n < smb )
         disp( [ ' ERROR in curly20: n = ', int2str(n), ' is less than ', num2str(smb), '!' ] )
      end
   else
      n = 30;
   end
...
case 'elobjf' % varargout = [ fiel, giel, Hiel ]
   ...
   switch( nargout )
   case 1
      varargout{1} = curly20( 'p4', sum(x) );
   case 2
      [ varargout{1}, gsum ] = curly20( 'p4', sum(x) );
      varargout{2} = gsum * ones( n, 1 );
   case 3
      [ varargout{1}, gsum, Hsum ] = curly20( 'p4', sum(x) );
      varargout{2} = gsum * ones( n, 1 );
