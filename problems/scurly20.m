...
function  varargout = scurly20( action, varargin )
...
case 'setup' % varargout = [ x0, fstar, xtype, xlower, xupper, clower, cupper, class, error ]
   ...
   if ( length( varargin ) )
      n = varargin{1};
      if ( n < smb )
         disp( [ ' ERROR in scurly20: n = ', int2str(n), ' is less than ', num2str(smb), '!' ] )
      end
   else
      n = 30;
   end
...
case 'elobjf' % varargout = [ fiel, giel, Hiel ]
   ...
   switch( nargout )
   case 1
      varargout{1} = scurly20( 'p4', sum(x) );
   case 2
      [ varargout{1}, gsum ] = scurly20( 'p4', sum(x) );
      varargout{2} = gsum * scale;
   case 3
      [ varargout{1}, gsum, Hsum ] = scurly20( 'p4', sum(x) );
      varargout{2} = gsum * scale;
