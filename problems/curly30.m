...
case 'setup' % varargout = [ x0, fstar, xtype, xlower, xupper, clower, cupper, class, error ]
   ...
   if ( length( varargin ) )
      n = varargin{1};
      if ( n < smb )
         disp( [ ' ERROR in curly30: n = ', int2str(n), ' is less than ', num2str(smb), '!' ] )
      end
   else
      n = 40;
   end
...
case 'elobjf' % varargout = [ fiel, giel, Hiel ]
   ...
   switch( nargout )
   case 1
      varargout{1} = curly30( 'p4', sum(x) );
   case 2
      [ varargout{1}, gsum ] = curly30( 'p4', sum(x) );
      varargout{2} = gsum * ones( n, 1 );
   case 3
      [ varargout{1}, gsum, Hsum ] = curly30( 'p4', sum(x) );
      varargout{2} = gsum * ones( n, 1 );
