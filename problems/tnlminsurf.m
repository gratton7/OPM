...
function varargout = tnlminsurf( action, varargin )
...
case 'setup' % varargout = [ x0, fstar, xtype, xlower, xupper, clower, cupper, class, error ]
   ...
   if ( length( varargin ) )
      ...
      if ( error )
         disp( [ ' ERROR in tnlminsurf: n = ', int2str(n), ' but should be a square and at least 9!' ] )
      end
   else
      n   = 16;
      nsq = 4;
...
case 'innerprod'
   ...
   if ( round (nsq) ~= nsq )
      varargout{1} = NaN;
      disp( ' ERROR in tnlminsurf-innerprod: dimension is not square!' )
      return
   end
   nsqm1 = nsq - 1;
   h2    = 1/nsqm1^2;
...
case 'M-matrix'
   ...
   if ( round (nsq) ~= nsq )
      varargout{1} = NaN;
      disp( ' ERROR in tnlminsurf-Mmatrix: dimension is not square!' )
      return
   end
   h2 = 1/(nsq-1)^2;
