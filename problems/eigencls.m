...
function  varargout = eigencls( action, varargin )
...
%
%   Example B: a trodiagonal matrix suggested by J. H. Wilkinson
%
...
case 'setup' % varargout = [ x0, fstar, xtype, xlower, xupper, clower, cupper, class, error ]
   ...
   if ( length( varargin ) )
      n = varargin{1};
      if ( n < 2 )
         disp( [ ' ERROR in eigencls: n = ', int2str(n), ' is < 2!' ] )
      end
      p = 0.5*(sqrt(1+4*n)-1);
      if ( abs( round( p ) - p ) > 1.e-15 )
         disp( [ ' ERROR in eigencls: n = ', int2str(n), ' is not p*(p+1)!' ] )
      end
   else
      n = 110;
      p = 0.5*(sqrt(1+4*n)-1);
